...
function get_git_branch(git_dir)
    ...
    --return io.popen("git branch --no-color 2>nul | grep '*' | sed 's/*//g' | sed 's/ //g' | sed 's/(//g' | sed 's/)//g' | awk -F / '{print $NF}'")
	
	--local t = io.popen("git branch --no-color 2>nul | grep '*' | sed 's/*//g' | sed 's/ //g' | sed 's/(//g' | sed 's/)//g' | awk -F / '{print $NF}' | tr -s '/n' ")
	--local result = t:read()
	--return result
end
